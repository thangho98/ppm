...
### Added
- **The System Monitor is now a full system monitor.** It had one page: a row of summary cards and a process table. Everything that makes Mission Center worth opening — which disk, which network interface, which GPU, what the memory is actually made of, which applications are running as opposed to which processes, and what systemd is doing — had no surface at all. The window now has five pages, and every number on them is measured without root.
  - **Performance** — a sidebar of every device on the machine, each with its own live sparkline, and a detail page per device. CPU: utilisation and kernel time, current, base and maximum clock, temperature, package power, threads, handles, uptime, sockets, cores, logical processors, virtualisation, L1/L2/L3 cache and the governor in force. Memory: the composition bar (in use / modified / standby / free), available, cached, committed, swap — and **the DIMMs themselves**, one row per slot with its manufacturer, type, form factor, speed and size, plus how many slots are populated out of how many the board has. Disks: active time and transfer-rate graphs, read and write speed, average response time, totals since boot, capacity, SSD-or-HDD, whether it is the system disk, removable, rotation rate and serial. Network: link speed and address per interface, typed as Ethernet, Wi-Fi, Bridge, VPN or Docker rather than listed as eight identical rows. GPUs: utilisation, video memory, encode/decode, clocks, power, temperature, driver, OpenGL, Vulkan and PCIe link. Fans: RPM, duty cycle and the temperature each one is answering to.
  - **Apps** — the applications you started, not the processes they spawned. A browser with nineteen renderers is one row with one icon, one CPU figure and one memory figure, ending as a group. They are identified from the systemd app cgroup each process lives in (`app-<desktop-id>.scope`), which is how the desktop itself knows, and the icon is the one that app's own `.desktop` entry names.
  - **Services** — every systemd unit, system and user scope kept as separate lists because a user unit and a system unit of the same name are different units with very different consequences. Filter by running, failed or enabled-at-boot; start, stop, restart, enable or disable one from its context menu behind a confirmation; open any unit for its state, main PID, user, unit file path and **this boot's log**, read from journald. Two `systemctl` calls serve a whole list rather than one per unit.
  - **Processes** gains Mission Center's own right-click menu: **Details…** (parent, user, state, threads, nice, start time, control group, executable, working directory, command line), **Send signal** (SIGTERM, SIGKILL, SIGSTOP, SIGCONT, SIGHUP, SIGINT, SIGUSR1, SIGUSR2) and End process — all of it refused, with the reason shown, for PPM's own processes, its tunnel, and anything OS-critical. Per-process **GPU** and **disk** columns are new too, read from DRM fdinfo and `/proc/<pid>/io`.
  - **The CPU graph is re-pointable, the way Mission Center's is.** Right-click it (long-press on a phone) for *Change top graph to* — overall utilisation, **one graph per logical processor**, all threads overlaid, or all threads stacked — and *Change bottom graph to* — temperature, package power, clock speed, or nothing. The per-thread grid is the one that earns its place on a hybrid part: a 12900K's eight P-cores, their SMT siblings and eight E-cores behave nothing alike, and a single averaged line says none of that. It lays out 24 threads in six columns of four exactly as Mission Center does — which is *not* `ceil(sqrt(n))`, that gives five — and drops columns rather than shrinking past legibility when the window is narrow or the screen is a phone.
  - Each graph now states what it is and where its top is — "Utilisation over 6 minutes / 100%", "Temperature over 6 minutes / 66.0 °C". The span is read from the points themselves rather than written into the label: PPM's window is 200 readings at the server's own cadence, which differs between the full and light tiers, so a fixed "over 1 minute" would be wrong on one of them and wrong again on a window that has only been open eight seconds.
  - Stacked mode divides each thread by the thread count, so the top edge of the stack is the machine's overall utilisation rather than 2400%.
  - **A drive's partitions, with what is actually on them.** Each disk page now lists its partitions with their label, filesystem, mount point and a used/total bar — the figures `df` prints, `statfs` arithmetic and all, so a btrfs root reads the same in both. One row per *partition* rather than per mount: a subvolume layout mounts one filesystem at seven places that all report identical numbers, and seven identical bars say nothing seven times. A partition that is not mounted says so; one whose usage the kernel refuses says *that*, with no bar, rather than drawing an empty one. The lookup was the hard part — `/proc/self/mountinfo` reports btrfs under an **anonymous** device number (`0:35` here) rather than the block device's own (`259:2`), so matching on that field alone finds no mount at all on most Arch installs, and the bug is invisible on ext4.
  - **Swap is a graph of its own, and zram is named.** The memory page drew one line and listed swap as a number underneath; swap now has its own graph against its own total, drawn only where there *is* swap — a flat zero line over an axis that does not exist reads as "nothing is swapping", which is a different claim from "there is nowhere to swap to". Beside it: how much **zram** is holding compressed and how much that saved, summed across every device, absent entirely on a host with none.
  - **The GPU page is three graphs, as Mission Center's is** — utilisation, video encode/decode and memory usage — each captioned with its span and its ceiling. And an integrated GPU finally has a memory figure instead of an em dash: i915 publishes no `mem_info_*` attribute, so the number is summed out of its clients' DRM fdinfo against the machine's RAM. Checked against Mission Center running beside it on the same host at the same moment: it read 2.52 GiB / 62.5 GiB, this read 2.54.
  - **Swap per process, and the Services page shows what each unit is actually using.** The process table, the app rows and the group roll-ups gain a **Swap** column from `VmSwap` in `/proc/<pid>/status` — 3.2 ms for 561 processes, beside the 1.7 ms `/proc/<pid>/io` already costs. Every unit on the Services page then carries live CPU, memory, swap, drive and GPU, summed over the processes in its cgroup. Those sums are the *same numbers the Processes tab is showing*, rolled up in the browser rather than recomputed in the Services route: that route is two `systemctl` spawns on a 3 s poll with no counters of its own, and computing CPU% there would mean a second set of deltas over a second interval — the same process reporting two different figures on two pages of one window. A unit is identified by **scope and name together**, because `dbus-broker.service` exists in both scopes on an ordinary desktop and one key for the two would publish one figure for two different units. A unit with no rows is a dash only when it OUGHT to have processes: systemd reports a mount as `active (mounted)`, a finished oneshot as `active (exited)` and a listening socket as **`running`** with an empty `MainPID`, so the row has to agree with the unit's own type — going by `ActiveState` dashed 97 of this host's 222 rows and going by `SubState` alone still dashed 14 sockets, where the honest reading is zero.

  - **Preferences** (the gear beside the connection light): temperature in °C or °F, and whether the CPU graph draws its kernel-time line. Both are per device — the unit you read in belongs to whoever is looking, not to the machine being watched — so a phone and a desktop watching the same host can disagree. Mission Center's update-interval and data-point-count settings are deliberately **not** here: PPM's tick rate is the server's and is shared by every subscriber, so a per-client control over it would either do nothing or change the rate for somebody else's window.
  - Everything is read from `/proc`, `/sys`, `/run/udev/data`, DRM fdinfo, RAPL and hwmon, and `systemctl`/`journalctl` for the units — no root, no daemon, no helper binary. A value the host cannot measure is **absent**, and an absent value renders as an em dash: the one thing a monitor must never do is state a measurement it did not take.

- **`bg-surface-hover` never existed, so every row hover and every meter track in the System Monitor was a no-op.** The class was written against `--surface-hover`, a token that only lives in the extension panels' injected stylesheet — in the app Tailwind emitted **no rule at all** for it, and a class that resolves to nothing reads exactly like a class that works. 21 uses in the System Monitor alone: no hover feedback on any Processes, Services or Apps row, and the memory composition bar, the partition bars, the Performance sidebar sparklines and the Overview per-core strip all painted their track `rgba(0, 0, 0, 0)` — measured at a contrast ratio of **1.00** against the card, i.e. 24 CPU cores drawn as floating stubs with no frame behind them, which is what made the card look broken rather than idle. Defined once in `globals.css` as `color-mix(in srgb, var(--text) 8%, transparent)`, derived from the text colour exactly as `webview-shell.ts` does it — never from a panel colour, because some themes give `--panel` and `--panel-2` the same value and a hover mapped from either is invisible in precisely those themes. `tests/unit/web/tailwind-token-exists.test.ts` now fails on any colour class in that directory naming a token `globals.css` does not define.

- **The Services page answered a 500 on macOS and Windows instead of saying it could not read them.** `Bun.spawn` does not report a binary that is not on `PATH` through an exit code — it **throws synchronously**, and in the shared `defaultRunner` that call sat outside the `try`, so the exception escaped a function typed to return a `Promise<RunResult>`. Every provider built on that runner shells out to a tool belonging to one OS (`systemctl`, `plutil`, `powershell.exe`, `findmnt`, `xdg-user-dir`), so "the binary is absent" is the *normal* case on any other host — and for the Services page it meant the route threw, answered 500, and the page rendered the exception text, while the collector underneath it has a `supported: false` path and the page has a branch that says "this host has no service manager PPM can read". A missing binary now comes back as a run that failed (`code: null`, the message on stderr), which is the shape every caller already handles for a non-zero exit. Reproduced by taking `systemctl` off `PATH` — before: `THREW → Executable not found in $PATH: "systemctl"`; after: `supported: false` with a warning naming the reason on each scope. It was invisible on Linux, which is the only platform this repository can run, and `tests/unit/services/host-info/spawn-runner.test.ts` now spawns a missing binary for real rather than stubbing the thing under test.

- **The Services page is a table with column names you can sort by.** It showed five figures per unit with no header at all — the labels were `title` attributes, i.e. a tooltip you had to already suspect was there — so the fourth number along was unidentifiable, and 222 units could only ever be read in one order. It now carries Mission Center's own eight columns in its order (**Name, PID, CPU, Memory, Swap, Drive, GPU, GPU Memory** — PID and GPU Memory are new, and GPU no longer shares one cell with its memory), each a sticky header that sorts on click: descending, ascending, then back off to the page's own order. That third state matters and is why the cycle has one: the default is failed first, then running, then alphabetical, which is the reason the page is usually opened at all, and a sort that could only be replaced would have taken it away. Two rules are inherited from the Processes tab rather than reinvented. **An unmeasured figure sorts last in either direction** — 189 of this host's units have no `MainPID`, and ascending by PID must not read as "these units have the lowest one"; a unit that owns no processes is a real 0 and sorts among the numbers. And ties break on the unit name, because the list is rebuilt from a fresh array every 2 s tick and the hundred-odd units that all read 0% would otherwise trade places on every one of them.
  - The columns arrive in a ladder on the panel's own width (`@container`, not the viewport — the window is routinely narrower than the screen): CPU and Memory first, then PID and Swap, then Drive, GPU and GPU Memory. All three of the header labels, the cell visibility and the grid template come from one table, because a column added to the header and forgotten in the template does not fail — it shifts every cell after it one track left, so the number under "Memory" is the swap figure and nothing says so. Verified by measuring: header and row templates identical, and all eight right edges matching to the pixel.

- **An Overview card opens the device it is about.** The five cards read a whole-machine figure each and the Performance page holds the per-device detail, but getting from one to the other meant changing tab, finding the device in a sidebar of twelve and clicking it — for a number you were already pointing at. Each card is now a button that switches to Performance with that device selected: CPU and Memory to themselves, a GPU card to its own GPU, and the Disk and Network cards to the **busiest** drive and interface, since those two cards show a machine-wide rate while the sidebar lists every device separately. Busiest rather than first, because `Disk 0` on this host is an idle Toshiba while the one doing the work is `Disk 1`, and by the metric each surface already prints — active time for a drive, raw throughput for an interface — so the page never contradicts the card that was clicked. A card with nowhere to go (a host reporting no drives at all) stays a plain frame rather than becoming a button that does nothing, and each button names itself ("Disk details") instead of letting a screen reader read the whole card as the control's name. On a phone the same tap lands *on* the device rather than on the list, which removed a piece of state rather than adding one: the panel no longer tracks whether it is drilled in, it asks whether a device is selected.

- **A dialog whose title is one long unbreakable string drew outside its own panel.** Opening a `.service` unit whose name is 60 characters with no spaces in it — `app-google\x2dchrome@8d02dd...service`, which is what systemd names a Chrome window — painted the title *over* the close button and ran the title and the log box past the right edge of the dialog, onto the page behind it. Measured: a 693px-wide column inside a 512px dialog. `DialogContent` is a CSS **grid**, and an implicit `auto` track takes its base size from its items' *min-content* contribution, so one 79-character log line widened the column and every child with it. Tailwind's `break-words` cannot prevent that — by specification `overflow-wrap` does not affect min-content size — so the column is now `minmax(0,1fr)`, whose base size is zero. Two smaller faults in the same dialog went with it: the close button is absolutely positioned over the header's first line, which now reserves that corner whenever the button is there; and the dialog's own `max-w-2xl` was **dead**, because the primitive caps width with a `sm:` variant that outranks a bare class at every width where it mattered — the dialog was 512px wide while its source said 672. `tests/unit/web/dialog-long-title.test.ts` pins all three.

- **The Overview cards are ruled like every other graph in the window.** `MetricChartCanvas` has carried Mission Center's 10x5 ruling all along and it defaults to off, which the five Performance detail pages all override and the five Overview cards did not — so an idle machine's CPU card was a flat line pinned near the bottom of an otherwise blank box, correct to the pixel (measured: ink at 86-89% down for a 7.4% reading) and reading as a rendering fault. The grid is what turns that empty space back into headroom.

- **A Mac with 8 GB of swap in use said nothing about swap at all.** `node:os` carries no swap figure, so off Linux `swapTotalMB`/`swapUsedMB` were simply absent — and the memory page renders an absent field as an em dash, i.e. as the claim that this host's swap cannot be measured. On macOS it can: `vm.swapusage` is one sysctl, and on the M1 Max this was found on it read **8039 MB of 9216 MB in use**, matching what the collector now reports to the megabyte on the same sample. Swap that is genuinely off stays a real `0` rather than becoming an em dash, because the UI hides the graph on a zero total and dashes only on `undefined`, and those are different claims. The cost was measured rather than assumed, since this runs on every tick: **1.33 ms**, against the 1.7 ms `/proc/<pid>/io` already costs per tick on Linux — and Linux pays none of it, because the `/proc/meminfo` branch returns before the reader is ever called.
  - Not changed, and deliberately: the *used* figure itself. `os.freemem()` on darwin looks alarming — 97.9% used on that Mac — and it is right. macOS's own `top` reported `31G used … 75M unused` on the same machine at the same moment. The tempting correction is to add `vm_stat`'s inactive pages, or to follow `memory_pressure`, which called that machine "56% free"; that metric measures what could be reclaimed under duress, not free RAM, and taking it would have replaced a figure matching Activity Monitor with one contradicting it. Recorded in the source so the next reader does not re-derive the wrong conclusion.

- **The Details dialog told macOS users that running processes were dead.** Opening it on a Chrome renderer using 4.2% of the CPU — visible in the table behind the dialog — answered **“PID 72973 is no longer running”**. The reader was `/proc`-only and the service answered `null` for every other platform, which the route turned into a 404 carrying that sentence: correct on Linux, where a vanished `/proc/<pid>` really does mean the process exited, and simply false anywhere else. The two claims now travel separately — a host with no reader answers **501** saying so, and “no longer running” is said only when a process actually is.
  - And macOS has a reader now, because it turns out `ps` answers **ten of the eleven fields**: parent, state, nice, user, executable, start time, threads, working directory, command line and name. Two of them arrive more directly than on Linux — `lstart` is a real timestamp rather than jiffies to be added to `btime`, and `user` is already the login name, so no `/etc/passwd` lookup is needed and it works across users. Only `cgroup` has no macOS equivalent and stays an em dash, which is the honest answer rather than an invented one. Cost, measured on an M1 Max: 29.7 ms for the first `ps`, then three concurrent calls (`ps -o command=`, `ps -M`, `lsof -d cwd`) at 30–38 ms — on-demand when the dialog opens, never on the tick.
  - Two traps are pinned by tests rather than left to be rediscovered. The parse is **positional**, because macOS `ps` offers no delimiter and two fields contain spaces: `lstart` is always five tokens (a single-digit day pads with two, which a `\s+` split collapses) and `comm` must be last, since a path like `/Applications/Google Chrome.app/…/Google Chrome Helper (Renderer)` holds six spaces. And the state letters are **BSD's, not Linux's** — uninterruptible wait is `U` here where Linux writes `D`, and `Z` is not an edge case: this host had 126 zombies against 645 sleepers.
  - Windows still has no reader; it now says that instead of claiming every process has exited.

- **"Up time —" on macOS and Windows, on hosts that knew perfectly well how long they had been up.** The CPU detail page takes `uptimeSec` from one writer, the Linux per-device collector reading `/proc/uptime`, and that collector is `null` on both other platforms — so the field was absent, and an absent field is drawn as an em dash, i.e. as the claim that the host cannot be asked. `os.uptime()` answers on all three (it reads `/proc/uptime`, `kern.boottime` and `GetTickCount64` respectively) and was used nowhere in the repository. It is now set while the snapshot is assembled, *before* `collectDevices` runs, so Linux's own finer reading is merged over it and that platform's number is unchanged to the decimal.

- **A core waiting on the disk no longer reads as a core at 100%.** The Overview card's per-core strip drew one bar solid red, every tick, for a thread that was 3% busy — and the number behind it came out of `os.cpus()`, which cannot express the state it was in. libuv's Linux `uv_cpu_info` reads the `iowait`, `softirq` and `steal` columns of `/proc/stat` into a throwaway variable, so a core parked in iowait shows a *tiny* total delta with no idle in it: measured on this host, 6 jiffies of which 0 were idle, while the kernel reported 195 of 201 in iowait behind a 191 MB/s write. Busy came out as 6/6. The sample is now taken from `/proc/stat` directly, where iowait counts as not-busy (as `top` and Mission Center both have it) and softirq counts as kernel time instead of vanishing from the total — `os.cpus()` remains the fallback for every other platform. Re-measured against the kernel across all 24 threads afterwards, the worst disagreement is a point or two of sampling skew and that core reads 4% against 4%.

- **The Overview GPU card no longer claims an integrated GPU has zero VRAM.** It printed "VRAM 0 MB / 0 MB" for a UHD 770, which is a different statement from "this device has no dedicated memory" — and the Performance page's own GPU detail already rendered that case as an em dash, so the two disagreed about the same number on the same host.

- **Event-loop lag is now measured, and attributed.** PPM serves every request, every WebSocket frame and all of the chat work from one process with one event loop, so anything synchronous holds up everything else for as long as it runs — which is why expanding a folder in the file explorer could sit on a spinner for seconds while a chat session was busy. That much was visible from outside (`/api/health`, which returns a literal, measured **18.9s** on a loaded instance against **0.12-0.65ms** on an idle one of the same binary), but it named no culprit, and nothing in PPM recorded it at all.
  - A monitor samples the loop four times a second and records every tick that slips, together with the process CPU consumed across the same gap. That second number is the point: a stall we spent *running* is our own synchronous work and worth hunting in this code, while a stall we spent descheduled is the machine being oversubscribed — and no change to PPM fixes that one, moving work to a worker thread least of all, since there was no CPU to move it onto. The two are indistinguishable without it.
  - Readable at `GET /api/system/event-loop`: how long the loop has been unavailable for, the worst single stall, and the recent ones with their cause and the resident set at the time.
  - It holds its samples in memory and writes nothing to disk. `~/.ppm/ppm.log` is already 261 MB with 54% of its lines exact duplicates, and a diagnostic that feeds the problem it diagnoses is not one.
...
- **Env files are highlighted, so a commented-out setting looks like one.** `.env`, `.env.local`, `.env.test.example` and every other variant opened as plain text — one colour for the whole file — which meant a line switched off with a `#` was indistinguishable from a live one. In a file whose entire purpose is "which of these are on", that was the only distinction the editor had to make and it made none. Comments are now dimmed, keys, values, `export` and `${INTERPOLATIONS}` each get their own colour, and `Ctrl+/` comments a line out.
  ...
  - The awkward lines were checked against real Monaco rather than reasoned about, and four of them were wrong at first: `#KEY=value` with no space, `export KEY=value`, a `#` inside a quoted value (not a comment), and `${VAR}` inside single quotes (literal, so not an interpolation). A fifth was worse — every line after the first assignment was being read as a value, including the commented ones.

- **An attached image can be opened before you send it.** The composer showed a 20px thumbnail and the filename, and clicking it did nothing — so the one attachment you would most want to check, a screenshot that was pasted and then silently downscaled, was the only thing in the composer that could not be looked at. The thumbnail is now a button that opens the same viewer the transcript uses: zoom, pan, rotate, flip, copy, download, and the arrow keys to walk a batch of pasted images without closing and reopening. It is reachable by keyboard, and on a touch screen the tap area is the full 44px while the thumbnail itself stays the size it was, so nothing in the composer moves.
...
- **A workspace folder whose *children* are the git repositories now works.** Point PPM at a folder of checkouts — a folder of services, a set of unrelated clones — and Source Control lists the repositories it found and asks which one to work in, instead of reporting "not a git repository" and leaving every git feature dead. The choice sticks per device and a quiet switcher in the panel header changes it; a single repository is chosen without asking, and a project folder that *is* a repository never sees any of this. Everything git-shaped follows the choice: status, staging by file or by hunk or by line, commit, push, pull, fetch, worktrees, the log, the diff viewer, inline blame and its hover, and the Git Graph extension's own panels — graph, blame, file history, compare, interactive rebase and reflog.
  - Discovery walks two levels down, skips `node_modules` and the other well-known caches, and does not descend into a repository once it has found one. A `.git` *file* counts as well as a directory, so a worktree or a submodule is listed rather than missed.
  - The scope is one query parameter resolved in a middleware, so every URL an ordinary single-repository project builds is byte-identical to what it was — the common case cannot regress on this path. It is validated against the project root: a path outside it is refused, never quietly replaced by the project folder, because that would answer with *a* history rather than with an error.
  - Paths cross the boundary in both directions. Git run inside the repository calls the file `src/a.ts`, while the tab that opens it calls the same file `frontend/src/a.ts` — so the file tree's decorations are moved into the project's terms while the panel's own list stays exactly as git reported it, since that list is what stage, unstage, discard and the hunk picker send straight back.
  - The Git Graph panels are opened on the repository, which means the extension can no longer find its project by matching that path exactly: it now resolves the innermost registered project *containing* the panel's root and rebases the file path before opening a tab. Without both halves, a diff opened from the graph named a project that does not exist and pointed at a file one directory too high — a spinner over an empty pane, with nothing to say why.

- **Real file icons, from the `vscode-icons` theme.** Every surface that lists a file — the explorer tree, the tab strip, Source Control, search results, the Problems panel, the command palette, the file picker and the OS file browser — now draws the same glyph VS Code draws for that file, resolved by whole filename first (`package.json`, `Dockerfile`, `bunfig.toml`), then double extension (`.spec.ts`, `.d.ts`), then extension. What it replaces looked deliberate but told you nothing: one lucide `FileCode` glyph tinted a different colour per language, so twenty languages were the same shape in twenty shades. A file icon's whole job is to be recognised before the name is read.
  - The tab strip and the dock, mobile tab switcher and mobile nav all label a file tab with *that file's* icon rather than a per-tab-type one. A strip of eight identical `FileCode` glyphs does not say which tab is which.
  - The artwork is vendored at build time (`bun scripts/gen-file-icons.ts`) rather than resolved at run time, which would mean shipping 1.3 MB of icon bodies and the extension's four lookup tables to draw a tree showing perhaps forty distinct glyphs. 224 glyphs come to 76 KiB brotli in a stylesheet of their own, because these are full-colour drawings with no `currentColor` to inherit and nothing to gain from putting the paths in the DOM — a tree of 500 rows would otherwise carry a few thousand extra nodes for React to reconcile on every expand. It is also how VS Code draws its own icon themes.
  - The theme draws some glyphs twice, and the mapping tables only ever answer with the *light-theme* one. Those are darker: the TOML glyph has no fill at all, so it was black on black on every dark PPM theme. Both drawings ship and the applied theme picks — which costs almost nothing compressed, since the two differ by a colour.

- **Fluent product icons for the app's own chrome.** The rail, toolbars, buttons, menus, status bar and mobile nav now draw Fluent System Icons at 20px Regular — the set `miguelsolorio.fluent-icons` puts over VS Code's product icons — instead of lucide. 207 of the 222 names the app uses have a Fluent equivalent; the other 15 stay on lucide and that is deliberate, because a *wrong* glyph is worse than an inconsistent one: there is no Fluent `git-commit`, no brand mark, and nothing that reads as "match case" or "use regular expression" for the search widget's toggles.
  - The two sets barely share a vocabulary, which is why this is a table and not a rename: lucide's `X` is Fluent's `dismiss`, `Check` is `checkmark`, `Trash2` is `delete`, `Plus` is `add`, `RefreshCw` is `arrow-sync`. Only 72 of the 222 names kebab-case straight across.
  - Icons are imported from `@/lib/icons` now, and a test fails if any file goes back to `lucide-react` — one file that does renders a stroked outline beside a filled one at the same size, which reads as "this button looks slightly off" rather than as an error.
  - The bundle got *smaller*: 953.7 kB → 933.7 kB (gzip 276.5 → 270.4), because 207 per-icon React components collapsed into 173 shared path strings behind one component. Glyphs stay in their native 20px box rather than being re-boxed to lucide's 24, which is what keeps an icon set from looking soft next to its own labels.
...
- **PPM ships its own typefaces: Geist for text, Monaspace for code.** Every stack in the app named fonts and then got none of them. `-apple-system`, `BlinkMacSystemFont` and `Segoe UI` all miss on Linux — the platform PPM is most often self-hosted on — and the generic fallbacks resolve through fontconfig to Liberation Sans, or for a bare `sans-serif` to Liberation *Serif*. So the interface was set in an Arial clone and the editor in whatever `fc-match monospace` happened to answer. `Geist` had been named in the stack since the theme engine landed and was never bundled.
  ...
  - **The terminal asks for MesloLGM Nerd Font first** — a shell prompt is full of powerline separators and devicons that only a patched font has — and where that is not installed it now falls back to a bundled set of the icons alone (see *The terminal prompt was a row of boxes on a phone*, below). A terminal that opens during the first paint used to keep whatever fallback was resolved then for the whole session, because xterm bakes the cell size and its glyph atlas from `ctx.font` at `open()`; it now re-measures once the face lands.
  - **The extension panels ask for the same typefaces by name.** A panel is a sandboxed iframe with an opaque origin, so it cannot use the faces the app bundles: a `@font-face` pointing at `/assets/` is a cross-origin fetch the font would need CORS headers to answer, and the file name carries the build's content hash the extension has no way to know. Naming them instead means the git graph and the app match exactly on a host where the fonts are installed, and fall back to the desktop's own UI font where they are not — which is still an improvement on the Arial clone it was getting. One token block, shared by the graph and every panel opened from it.
...
### Fixed
- **A folder you expand in the explorer stays where it is.** Expanding a directory that was its parent's only child made the row you had just clicked *disappear*: it was joined onto its parent as one `.claude/agent-memory` line, and everything beneath it shifted a level left. VS Code does join single-child chains, but on the way *in* — you never see the intermediate row, so there is nothing there to vanish. PPM's version only joined once *both* rows were already expanded, so it fired one gesture too late, and the click that should have revealed a folder removed one instead. Every directory now gets its own row at its own depth, so the indentation reads as nesting again.

- **Gitignored files were unreadable in the explorer on the light themes.** The tree dims them, and it did that by fading the whole row to 40% opacity — which reads as one design decision covering both modes and is not one. Fading a near-white name on a dark panel keeps a usable gap; fading a near-black one on white collapses it. Measured against PPM's six built-in themes, a gitignored filename sat at **1.72–1.89:1 against the panel on the three light themes** and 2.07–2.23:1 on the dark ones: the first of those is not low contrast, it is a name that cannot be read, which is why a whole `.claude/` subtree looked blank on light and perfectly fine on dark. The state is now a *colour* rather than a fade — **3.59–5.21:1 across all six**, still clearly dimmer than an ordinary file — and the file icon keeps its own colours, because those glyphs are full-colour artwork with no `currentColor`, so any opacity on them is that same fade to the background.

- **A bug report no longer stalls the server for a third of a second, and anyone could ask for one.** `GET /api/logs/recent` returns the last 30 lines of `~/.ppm/ppm.log` so the login screen can show something useful when things go wrong. It did that by reading the whole file and splitting it: on the 276 MB log this was found on, **264 ms to read plus 92 ms to split — 356 ms of a completely blocked event loop and a 644 MB resident spike, to answer with 30 lines.** The cost grew with the log, and the route is registered *before* the auth middleware, so it could be asked for repeatedly by anyone who could reach the port. It now reads a bounded 64 KB from the end: **0.48 ms** on the same file, and the same on a file ten times the size.
  - It also answers with 30 lines now rather than 29. A file ending in a newline splits to a trailing empty string, which counted against the total and then got trimmed away — the old spelling got away with it only because it always asked for 30.

- **Secrets were being written to `ppm.log` unredacted, by a second path nobody had noticed.** The supervisor spawns the server with `stdio: ["ignore", logFd, logFd]` where `logFd` is `ppm.log` itself, so every `console.log` already reached the log through file descriptor 1 — raw, with no timestamp and, critically, without passing through `redactSecrets()`, which only ever ran on the *appended* copy. The log held 1,395,012 lines of which only 532,792 carried the `[timestamp] [LEVEL]` prefix: the other **862,220 (62%) were the same events arriving unformatted and unredacted**, including lines matching `Token: <value>`. Where the console already reaches the file, the original call is now dropped and only the formatted, redacted line is written — one copy per event, and the safe one. Run from a terminal (`bun dev:server`) nothing changes: stdout is a tty, and both the terminal and the log get their line as before.

- **`ppm.log` is now bounded.** It had reached 276 MB and was still growing. It is checked once a minute against a 20 MB cap and rotated through three generations, by the supervisor alone — it owns the descriptor every child was handed, and two processes truncating one file would race. Rotation *truncates in place* rather than renaming, which is the only thing that works here: the supervisor and every child hold descriptors opened on that inode with `O_APPEND`, and a rename would leave all of them writing into `ppm.log.1` for the rest of the process's life, so the log would appear to rotate once and then never grow again.

- **Opening a project no longer freezes the whole server for two to four seconds.** Registering a project's file watchers walked the directory tree with `readdirSync` and then called `fs.watch` on every directory it kept — both synchronous, both in one burst. On a 12,000-directory project that held the event loop for **2.95 seconds**: no request served, no WebSocket frame read, no chat token forwarded, for anyone, in any project. It was found by lining a lag sample up against `ppm.log` — a 2.95 s silence ending exactly on the `Started watching: … (12000 dirs)` line — and confirmed on two independent restarts at ratio 1.38, the ratio being over 1 because Bun runs those `readdirSync` calls on its file system thread pool while the main thread walks.
  - The walk now hands the loop back every 64 directories. Measured on a 13,321-directory tree with a 5 ms timer running alongside: before, that timer fired **0 times** in 2.1 seconds; after, **174–189 times**, with the worst single pause down to **35–50 ms**. Wall time went from 2.1 s to 2.2–2.8 s, which is the trade being made on purpose — the walk is slower and the server stays answerable throughout.
  - 64 was picked by sweeping it, not guessed: every 250 directories leaves a 134 ms pause, every 32 gets to 23 ms but costs 1.8 extra seconds of walking. The yield is `setImmediate`, which reaches the same 41 ms worst pause as `setTimeout(…, 0)` for 600 ms less wall time, and it has to be one of those two — `await Promise.resolve()` drains as a microtask and would leave the loop exactly as blocked while looking asynchronous.
  - **What this changes for callers:** `startWatching()` no longer has the tree attached by the time it returns, so a change made in the window right after it is not reported. It returns a promise that resolves when coverage lands, and anything that touches the filesystem immediately afterwards has to await it. The walk also checks whether it has been closed after every pause — closing a project mid-walk used to be impossible and now is not, and a walk carrying on past it would attach watchers nothing will ever release.

- **Session debug info no longer holds the event loop while it counts a transcript.** `GET /chat/sessions/:id/debug` reports how many records a transcript has, and it worked that out by reading the whole file into a string and walking it with `charCodeAt` — 53 ms on a 35 MB transcript, up to ~95 ms at the 64 MB cap, during which nothing else in PPM could run. It now counts newline bytes off a stream. This is **not faster** (also 53 ms), which is the point worth stating: what changed is that the loop gets the thread back nine times during it, worst gap 11.8 ms, where before it got it back zero times. Measured with a 5 ms interval running alongside; awaiting the stream alone was *not* enough, because its chunks resolve as microtasks and a microtask queue drains without ever letting a timer run.

- **The file explorer stuttered when scrolled on a tablet or phone.** The tree was already virtualized, so this was never about how many rows exist — it was that the virtualizer's default mode ends every scroll event in a synchronous React re-render of every row on screen, *inside the scroll handler*. Measured over 1191 rows on a 6x-throttled CPU: 92ms of a 137ms long frame was the scroll listener, while style and layout together came to 4ms. The virtualizer now writes each row's position straight to the DOM and React only renders when the set of visible rows actually changes — the same division of labour VS Code's list has, where scrolling moves one container and leaves the rows alone.
  - Frame time over the same scroll, same throttle: median **38.2ms → ~25ms**, frames missing 30fps **76 of 80 → 1**, long animation frames **2 → 0**. On a desktop it was already 60fps before and still is; this is a fix for the devices that were not.
  - A row is also exactly the height the virtualizer is told it is now. 13px text with the inherited line-height made a 27.5px row against an estimate of 26, so every row scrolling into view corrected the total height underneath the scrollbar.
  - And the row component's memo works: its action handler was rebuilt on every render, so the memo never once hit and rows re-rendered whatever had changed.
  - **One context menu for the whole tree**, the way VS Code's explorer has a single `onContextMenu` rather than a menu per row. Every mounted row was constructing this menu's 39 items on every render, and subscribing to a fifth store purely to feed it; which row the menu belongs to is now resolved from whichever one the click or press landed on. Frame time went the rest of the way: median **~25ms → ~18.5ms**, and no frame in three runs missed 30fps.
  - A bug the first of these shipped, found and fixed: expanding a folder left the rows below it drawn on top of each other, at the offsets they had before the expansion. Writing positions to the DOM means the virtualizer has to find each row's element, and it looks one up by the row's *index* while React tracks it by its *path* — so a row that only moved down the list was never re-registered, and the newly inserted rows took over the slots it had. Both now identify a row the same way.
  - **Clicking a folder no longer throws away the request that was already fetching it.** The tree fetches one directory level at a time and quietly prefetches the level ahead, so the next click renders from memory — but a click arriving while that prefetch was still in the air *aborted* it and asked for the identical thing again, which is the click the prefetch existed to make instant. It waits for the answer already coming now. Nothing else changes: the URL and the response were always the same, so there was never anything to preempt.
- **The Git Graph's commit details panel could not be closed on a touch screen.** Tapping a commit opens a panel over a third of the graph, and the only ways out were the Escape key — which a phone does not have — and tapping the same commit row again, which by then is behind the panel. The two hash chips at the right of its header are now a close button. Nothing is lost by dropping them: the metadata grid two lines below carries the commit and its parents in full and copies either on a click, and the chips were hidden on a phone anyway, where two of them took half the header and left the author's name as "t." with an ellipsis — so that corner of the header now has something in it at the width where closing matters most. Escape still works, and both go through the same dismissal, which also clears the row's highlight and its mark in the scroll bar.
- **The Source Control file tree told you almost nothing about which files had changed.** Names were cut at the *end*, and these names are distinguished by their suffix: `remote-desktop-capture.ts`, `remote-desktop-capture-input.ts` and `remote-desktop-encoder-args.ts` are identical for their first 23 characters, so seven rows in a row read `remote-desktop-…` and the panel could not tell you what it was listing. The names now ellipsize from the *start* — `…p-capture-input.ts`, `…esktop-capture.ts`, `…p-encoder-args.ts` — which is the part that differs, and the full path is still on the row's tooltip.
  - **A chain of directories with one child each is one row**, the way VS Code's explorer folds them: `src` → `services` → `remote-desktop` cost three rows and 36px of the width the filenames needed to convey a single path, and is now `src/services/remote-desktop` on one line with the leading part dimmed so the specific directory stands out. Staging or discarding that row still acts on the deepest directory, not the outermost one.
  - **The tree's connector lines are drawn per level instead of per row.** They had been an elbow positioned by hand inside each row — one offset for a file, another for a folder, against rows of two different heights — so the pieces never met and the result read as scattered dashes rather than as a tree. One continuous rail per level now runs the height of its children.
  - Rows are 44px tall on a touch screen and compact where there is a mouse, and a filename is set at the body text size rather than the size used for labels and metadata.

- **The typing indicator on a streaming chat tab was a smudge, not an animation.** A tab whose reply is still arriving draws three bouncing dots inside its chat-bubble icon, Messenger style. That was written against the old icon set, whose speech bubble is hollow. The bubble PPM ships now draws **two message lines of its own** inside itself — and the dots are laid over the same 16px box in the same colour, so they landed on top of the upper line and the three of them fused into one lumpy bar. At the size a tab actually renders it read as a static blob: the dots were still bouncing, but there was nothing legible left to move.
  - The streaming state now uses the empty bubble from the same icon family — the identical outline with nothing in it — so the dots have the inside of the bubble to themselves. The resting icon is unchanged, and the two are the same shape, so a tab does not appear to swap icons when a reply starts.
  - Checked by freezing the animation at five points across its cycle and rendering both: the old glyph barely changes shape between them, the new one shows three dots plainly rising and falling. A test now asserts the glyph behind the dots contributes nothing but the bubble's outline and its hole, so an icon with its own contents cannot quietly end up back there.

- **Half the shell prompt was invisible on a light theme.** A terminal palette has sixteen colour slots and a background, and two of those slots are *defined* to sit at the extremes — so on a light theme `white` and `brightWhite` land right on top of the background (measured: **1.09:1** and **1.07:1**), and on a dark theme `black` does (**1.10:1**). That is not low contrast, it is nothing at all. oh-my-posh writes its second prompt line in plain ANSI white, which is correct on the dark terminal it was written for, so on a light PPM theme that whole line — the clock, the folder, the path — simply was not on the screen.
  - Recolouring those slots would have been the wrong fix, because the same `white` is *also* the text on a coloured powerline segment, where white is exactly what it should be. The terminal now asks xterm to hold a minimum contrast ratio of 4.5 (WCAG AA, and VS Code's own default), which adjusts the foreground per cell against whatever is actually behind it: every coloured segment renders identically, and only the text that had the bare background behind it moves. Checked by rendering the real prompt in both themes side by side.

- **The Source Control panel's row menus, on a phone.** Long-pressing a changed file opened a desktop dropdown: a small popper anchored to a 20px row, with items sized for a mouse pointer. It is now the same bottom sheet every other long-press in PPM opens — anchored to the bottom of the screen where the thumb is, with a backdrop, 44px rows, and Discard Changes set apart in red below a divider. On a desktop the same menu is on right-click, which the panel did not have at all before.
  - **A folder row could not be expanded on a phone.** Its menu was a plain dropdown whose trigger was the whole row, and a dropdown trigger opens on *tap* — so tapping a folder opened a menu over it instead of expanding it, and nothing else would. Tap now expands and long-press opens the menu, which is the gesture the rest of the app already used.
  - Tapping a file still opens its diff, but the filename is a real button doing it rather than a hand-rolled "was that a tap or a press?" timer — so it is reachable by keyboard, and a press that the browser turned into a scroll can no longer be mistaken for a tap. Both menus are one definition now instead of a desktop copy and a mobile copy.

- **The terminal prompt was a row of boxes on a phone.** A themed prompt — oh-my-posh, powerlevel10k, starship — draws its segment separators, its git status and its language badges from Private Use Area codepoints that only a *patched* font has. PPM named three of those fonts and shipped none, so on any machine without one installed every icon in the prompt was a box or a question mark, while the `↑39 ↓15` sitting between them rendered perfectly — because those two arrows are real Unicode and everything around them was not. Desktops mostly got away with it, since a machine with a Nerd Font installed supplies the glyphs itself; a phone has nothing to fall back to.
  - The icons are now bundled, and only the icons: no letters, no digits, so ordinary text falls straight through to the code face and only the icon codepoints stop at the new one. A fully patched font installed on the host still wins, because it draws the icons *and* the letters in one typeface at metrics fitted together.
  - The whole set is 1.14 MB, which is why it ships as **fourteen** files split along the icon-set boundaries — Powerline, Codicons, Devicons, Font Awesome, Material Design and so on — each declaring the exact range of characters it covers. A browser fetches one only when it actually lays out a character inside it, so a powerline prompt costs **7 KiB**, a prompt with git and language segments about 210 KiB, and the 492 KiB of Material Design icons is downloaded by whoever draws one and by nobody else. Nothing enters the service worker's precache, and the boot shell in fact got 9 KiB *smaller*: the bundler had been quietly base64-ing the four smallest faces into the one file that is precached, which turned the rarest icon sets into an unconditional download for everybody.
  - A face arriving late had to be made to draw. The fetch does not start until something renders one of those characters, which for a prompt is long after the page has settled — and measured in a real terminal, the tofu it drew first is *permanent*: the renderer bakes glyphs into a texture atlas, and nothing repaints it, so the prompt stays wrong for the rest of the session with the correct font sitting loaded in the page. The atlas is now discarded and the screen redrawn whenever a font finishes loading.
  - The font is vendored by a script that pins the upstream version and checks the file against a digest before subsetting it, because a moved tag or a replaced release asset would shift every glyph in the terminal with nothing to point at.

- **Context menus that opened by themselves while you were scrolling on a phone.** Scroll the list of changed files, the tab switcher, or the project list, and a menu would appear over it — nothing you did asked for one, and the list was still moving under your finger. A press that opens a menu is a timer started when the finger lands and cancelled when it moves or lifts, which reads as complete and is not: once the browser decides the gesture belongs to a scroll it *cancels* the touch and stops reporting it, so the timer was never told and finished on its own. It is now cancelled on that event too, in all seven places that hold a press open — the file rows, the shared context menu, the file explorer's rows, the tab strip's drag, the drawer tab bar, the tab switcher and the project sheet. The press is also dropped if what you pressed disappears first, which the git panel does on every file save.

- **The occasional white screen.** Sometimes a page — a file you opened, a session you came back to — rendered nothing at all: no message, nothing to click, just the background. It was always the same thing underneath. PPM's frontend is split into content-hashed files, an upgrade replaces every one of them, and a tab that was open across the upgrade goes on asking for the names it was built with. Four separate faults turned that into a blank page instead of a hiccup.
  - **The server answered a missing script with the app's own HTML.** Any path it could not find fell back to `index.html`, which is right for a page address and wrong for a file the running app is fetching: the browser refuses to run HTML as a module, so the load failed with a message about MIME types that named nothing recognisable. Missing files now come back as a plain 404. The test cannot be the file extension — PPM's addresses contain file paths, so `/project/x/editor/src/main.js` is a page and not a script — so it is the browser's own statement of what the request is for, with the two build-output directories as a backstop for the connections where that statement is not sent.
  - **One failure took the whole app down with it.** A tab type that will not load throws while React is rendering it, and with nothing above it to catch that, React unmounts everything — which is exactly a blank page. There is now a boundary at the root, so the same failure reads "PPM has been updated" with a Reload button, and one attempt at recovering silently happens first: this app's caches are dropped and the page reloads itself once, which in practice fixes it before you notice. Once, not repeatedly — a page that reloads forever is worse than one that explains itself.
  - **Some failures happen before any of PPM's code runs.** The app's startup depends on around forty of those hashed files, and if one is missing the browser never executes any of it — so no boundary, no handler and no message is possible, because nothing that could show one has loaded. That case is now caught by a few lines built into the page itself, which say what happened and offer the same Reload. It stands down the moment the app starts normally, so a slow phone never sees it.
  - **The service worker made all of it permanent.** It had cached that HTML response under the script's own address, and since these files are treated as never-changing, it kept serving it: putting the file back on the server changed nothing, and the tab stayed broken until its storage was cleared by hand. It now refuses to store — or to keep serving — a page where a script belongs, drops superseded caches instead of accumulating them, and an updated PPM takes effect on the next load rather than waiting for every tab to close, which is what had let a tab run last week's code for a week.
  - Separately, a mistyped or truncated address with a stray `%` in it crashed the app on load instead of simply not opening anything.

- **Chat search could not find a session by the name you gave it, and never looked inside the turns that only used tools.** Two unrelated faults behind one symptom: searching for a session's own title returned nothing, and searching for something a tool had done returned nothing either.
  - Only the 50 most recent sessions were listed under their real name. PPM asked the SDK for a single page of sessions, and everything past it fell through to the recovery scan meant for transcripts the SDK cannot parse — which can only rebuild a title by scraping the first 512 bytes of the file. So 165 of 228 sessions in one workspace were listed under their opening prompt ("/recap") instead of the name they had been given, and a search for that name could not match them. A project-scoped listing with no explicit limit now pages the SDK until it is exhausted, leaving the scan the exception path it was written to be; the paginated session list is untouched.
  - A turn that only used tools was dropped from the index entirely. Tool calls and their results are carried separately from a message's prose, and the indexer read the prose alone — so the turns that commit and open a pull request, the most tool-heavy there are, were the least searchable part of a session rather than the most. Two sessions measured here went from 33 and 17 indexed messages to 122 and 76. Tool names, their arguments, their output and nested agent events are all indexed now, capped per event because a single file read can be hundreds of KB. The model's thinking stays out: bulky, and it reads as noise in a result snippet.
  - Neither fix would have reached a transcript already indexed. Staleness was the transcript's modification time and nothing else, so every session was stamped as fresh and a better indexer would never have been asked to re-read one. The index now records which indexer wrote each session and treats a change of indexer as stale, which re-reads all 256 already on this machine.
  - Still missing, and not PPM's to fix: the Claude Agent SDK returns only part of a long transcript, so a session's earliest messages remain unsearchable — one here spans 02/09 11:16 to 03/09 11:03 in the file and is answered from 03/09 05:09 onwards.
